...
import title.Continue;
import title.No;
import title.Yes;

public class ContinuePanel extends JPanel implements Runnable{
	...
	// Password
	public PasswordContinuaPanel password;
	...
	// Variabile che indica il livello in cui si è arrivati che viene restituito quando l'utente preme yes per ricominciare a giocare
	public int level = 0;
	
	// Pannello da cui prendere le informazioni per il salvataggio
	GamePanel gp;
	
	// Oggetto per il salvataggio della partita
	SaveLoad saveLoad;
	
	// Costruttore della classe
	public ContinuePanel(GamePanel gp) {
		...
		titleContinue = new Continue(this);
		titleYes = new Yes(this);
		titleNo = new No(this);
		cursorPage = new ContinuePageCursore(this, keyH);
		password = new PasswordContinuaPanel(this);
		this.gp = gp;
		saveLoad = new SaveLoad(this.gp, this);
		getImagePage();
	}
	...
	public void update() {
		...
		if(keyH.enter || keyH.esc) {
			cursorPage.select.start();
			saveLoad.save();
			
			if(keyH.enter && cursorPage.title.equals("yes")) { 
				level = saveLoad.load().level;
			}
	...
	public void paintComponent(Graphics g) {
		...
		// Disegno del titolo Continue?
		titleContinue.draw(g);
		
		// Disegni dei titoli Yes/No
		titleYes.draw(g);
		titleNo.draw(g);
		...
		// Disegno dei numeri della password
		password.draw(g);
	...
	public int createPanel() {
		...
		window.add(this);
		window.pack();
		return start(window, this);
	}
	
	// Rende visibile la finestra al centro dello schermo avviando il thread
	public int  start(JFrame window, ContinuePanel mp) {
